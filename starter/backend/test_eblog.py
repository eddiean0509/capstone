...
class TestAPI(unittest.TestCase):
    ...
    def test_post_delete(self):
        ...
        # 404, invalid_post
        invalid_post = 9999
        response = self.client().delete(f"{BASE_URL}/{invalid_post}/", headers=headers)
        self.assertEqual(response.status_code, 404)
    ...
    def test_reply_modify(self):
        ...
        invalid_reply = 9999
        response = self.client().put(
            f"{BASE_URL}/{post_id}/reply/{invalid_reply}",
            headers=headers,
    ...
    def test_permission(self):
        ...
        # neighbor does not have permission for post
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {NEIGHBOR_TOKEN}"}
        data = {"subject": "Test Post", "content": "This is a test post."}
        response = self.client().post(BASE_URL, headers=headers, data=json.dumps(data))
        self.assertEqual(response.status_code, 403)

        # neighbor have permission for reply
        post_id =1
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {NEIGHBOR_TOKEN}"}
        data = {"content": "This is a test reply."}
        response = self.client().post(
